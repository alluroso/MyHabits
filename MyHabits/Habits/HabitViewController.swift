...
class HabitViewController: UIViewController {
    ...
    private lazy var setTimeLabel: UILabel = {
        let label = UILabel()
        label.text = "\(currentTime) \("PM")"
        label.textColor = .purpleCustom
        label.font = .body
        label.translatesAutoresizingMaskIntoConstraints = false
        return label
    }()
    ...
    private func setupNavigationBar() {
        if habit == nil {
            ...
            let leftBarButtonItem = UIBarButtonItem(title: "Отменить", style: .plain, target: self, action: #selector(onCancelButton))
